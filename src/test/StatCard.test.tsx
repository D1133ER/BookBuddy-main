...
describe('StatCard', () => {
  it('renders title and numeric value', () => {
    render(<StatCard title="My Listed Books" value={12} icon={<BookOpen data-testid="icon" />} />);
    expect(screen.getByText('My Listed Books')).toBeInTheDocument();
    expect(screen.getByText('12')).toBeInTheDocument();
  });

  it('renders string value', () => {
    render(<StatCard title="Approval Rate" value="87%" icon={<BookOpen />} />);
  ...
  it('does not render trend section when trend is not provided', () => {
    const { container } = render(<StatCard title="Title" value={5} icon={<BookOpen />} />);
