...
const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.errors?.[0]?.message || err.message || "Failed to register. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <PageTransition className="min-h-screen bg-white">
        ...
        <div className="mx-auto w-full max-w-sm lg:w-96">
          ...
          <div className="mt-8">
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <Button type="submit" className="w-full h-11 rounded-xl text-base font-medium mt-2" disabled={isLoading}>
      ...
      <div className="hidden lg:block relative w-0 flex-1 bg-gray-50">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-16 text-white">
          <h3 className="text-4xl font-bold mb-4">Start a shelf your neighbors will actually use.</h3>
          <p className="max-w-xl text-lg text-white/95">
            Create a local-first account, list the books you are willing to lend, and keep every request, pickup, and return in one place.
          </p>
        </div>
      </div>
      </main>
